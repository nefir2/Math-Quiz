...
namespace Math_Quiz
{
	public partial class Form1 : Form
	{
		...
		public void StartMQ()
		{
			...
			timeleft = 6000; //таймер увеличен с 30 на 60, так как добавлен сложный пример.
		...
		private void timer1_Tick(object sender, EventArgs e)
		{
			if (CheckAns()) //если все ответы верны:
			{
				...
				string speedrun = 60 - timeleft / 100 < 10 ? "0" + (60 - timeleft / 1000) + "." + timeleft % 1000 : $"{60 - timeleft / 100}.{timeleft % 100}"; //если число оставшегося времени - цифра,
																																								  //то подставляется 0 перед этой цифорй.
				timelabel.Text = $"спидран: 0:{speedrun}"; //установка текста после завершения теста.
				EndTest(); //сток, после окончания теста.
			}
			else if (timeleft > 0) //иначе если время ещё осталось:
			{
				timeleft--; //вычитается секунда из таймера,
				timelabel.Text = $"{timeleft / 100} секунд."; //вывод оставшегося времени.
			}
